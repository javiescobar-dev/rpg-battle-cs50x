...
launcher muestra las noticias del proyecto, un carrusel de imágenes, y permite
...
pixel art (castillo, bosque, cueva, puerto). Catorce efectos
de sonido WAV se asocian a las acciones y suenan en las fases concretas
(arremetida, lanzamiento, impacto), con blips de cursor en los menús. Las
...
son botones propios, y la cabecera se puede arrastrar.
...
conocida si no hay conexión). Cada noticia se muestra
como un slide con su imagen (recortada sin deformar para llenar el marco),
overlay, título y cuerpo, con flechas y puntos de navegación; el cambio de slide
se anima con una transición deslizante.
...
date" (deshabilitado) si ya está al día. Durante la descarga muestra una barra
...
redes corporativas o máquinas virtuales donde el antivirus intercepta HTTPS. Las
rutas de datos usan `platformdirs` (el directorio de datos del usuario de cada
sistema operativo), nunca la carpeta del ejecutable.
...
- **Atribución de recursos.** Los sprites provienen de Pixel Champions II y los
  fondos son escenas de pixel art generadas con ayuda de IA; el uso de la
  herramienta de IA que acompañó el desarrollo se cita en la cabecera de cada
  archivo fuente, como pide la política del curso.
